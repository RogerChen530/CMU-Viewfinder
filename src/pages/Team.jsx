...
export default function Team({ user, role }) {
  ...
  const canSeeEmail = !!user && (role === "member" || role === "admin");

  useEffect(() => {
    setLoading(true);
    const table = canSeeEmail ? "member_directory" : "public_member_directory";
              ...
              <div key={m.id} className="border border-seam rounded overflow-hidden flex flex-col">
                ...
                <div className="p-2.5 text-center">
                  ...
                  <div className="mt-1.5 flex flex-col gap-0.5 text-[11px] text-ash">
                    {canSeeEmail && m.contact_email && <span className="truncate">{m.contact_email}</span>}
                    {m.ig_id && <span>@{m.ig_id}</span>}
                    {!(canSeeEmail && m.contact_email) && !m.ig_id && <span>—</span>}
                  </div>
                </div>
              </div>
            );
